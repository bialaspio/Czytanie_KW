					...
					if 'Rubryka 2.3' in th_tabelka:
						...
						for td in tr.findall('td'):
							...
							if insert_wiel_udzl_bud_lok ==1111:
								insert_query = "insert into udz_lok_bud_Kamienica(NR_KW,nr_udzialu,wiel_udzialu_bud,nr_kw_bud,nr_lok_bud) VALUES ('"+nr_ksw+"','"+nr_udzialu+"','"+wiel_udzialu_bud+"','"+nr_kw_bud+"','"+nr_lok_bud+"')"  
								insert_wiel_udzl_bud_lok = 0
								try: 
						...
						if '1. Obszar' in td.text:
							...
							for td_local in tr:
								if licznik == 3:
									...
									print 'Obszar : '+ obszar
									insert_query = "insert into pow_sum_dzial_Kamienica(NR_KW, Pow_sum_dzialek) VALUES ('"+nr_ksw +"','"+obszar+"')"
									exec_query_commit_PG(insert_query)						
...
#										nr_udzialu = '1'
									...
									insert_query = "insert into udzialy_Kamienica(NR_KW, NR_udzialu,Udzial) VALUES ('"+nr_ksw +"','"+nr_udzialu+"','"+wielkosc_udzialu+"')"
									exec_query_commit_PG(insert_query)						
							...
							for td_local in tr:								
								if licznik == 3:
									...
									if insert_osoba==11111:
										insert_query = "insert into wlasciciele_Kamienica(NR_KW, NR_udzialu,Imie,Drugie_imie,Nazwisko_1,Nazwisko_2,Ojciec,Marka) VALUES ('"+nr_ksw +"','"+nr_udzialu_oso+"','"+pierw_imie+"','"+drugie_imie+"','"+nazwisko_pierw_cz+"','"+nazwisko_drugi_cz+"','"+imie_ojca+"','"+imie_matki+"')"
										exec_query_commit_PG(insert_query)						
										insert_osoba = 0
								licznik = licznik+1				
						#instytucja 
						if '2. Nazwa' in td.text:
							...
							for td_local in tr:								
								if licznik == 3:
									...
									insert_query = "insert into wlasciciele_Kamienica(NR_KW, NR_udzialu,Imie,Drugie_imie,Nazwisko_1,Nazwisko_2,Ojciec,Marka) VALUES ('"+nr_ksw +"','"+nr_udzialu_oso+"','"+instytucja+"','','','','','')"
									exec_query_commit_PG(insert_query)						
								licznik = licznik+1
									...
									if licz_tr == 11:
										...
										for td in tr.findall('td'):
											if licz_td == 3:
												...
												##	insert_query = "insert into dzialki_Kamienica(NR_KW, NR_dzialki,Pow_dzialki, Pow_suma_dz) VALUES ('"+nr_ksw +"','"+nr_dzialki+"','"+pow_dzilki+"','"+obszar+"')"
													insert_query = "insert into dzialki_Kamienica(NR_KW, NR_dzialki,NumerObrebuEwid,NazwaObrebuEwid,Pow_dzialki,pol_dzialki,plik) VALUES ('"+nr_ksw +"','"+nr_dzialki+"','"+nr_obr_ewid+"','"+NazwaObrebuEwid+"','"+pow_dzilki+"','"+pol_dzialki+"','"+xml_file_name+"')"  
													exec_query_commit_PG(insert_query)						
													dzialka_insert = 0
											licz_td = licz_td+1
										licz_tr = 1	
				...
				if TABLE_ID == 'Budynki': 
					...
					for tbody in table.findall('tbody'):
						...
						if TBODY_ID == "BudynekKW":
							...
							insert_query = "insert into budynki_Kamienica (NR_KW, IdentyfikatorBudynku_par01,IdentyfikatorBudynku_par02,przeznaczeniebudynku, ident_dzi,LokaleWyodrebnioneNr,	LokaleWyodrebnioneKw,Ulica,NrPorzadkowy) VALUES ('"+nr_ksw +"','"+IdentyfikatorBudynku_par01+"','"+IdentyfikatorBudynku_par02+"','"+PrzeznaczenieBudynku+"','"+ident_dzialki+"','"+LokaleWyodrebnioneNr+"','"+LokaleWyodrebnioneKw+"','"+Ulica+"','"+NrPorzadkowy+"');"
							print insert_query
							exec_query_commit_PG(insert_query)
							
def creae_tables ():
	# dziala z numerem i powierzchnia 
	drop_quer = "drop table if exists dzialki_Kamienica;"
	exec_query_commit_PG(drop_quer)
	create_query = """create table dzialki_Kamienica (
	 NR_KW varchar,  
	 NR_dzialki varchar,
	...
	drop_quer = "drop table if exists pow_sum_dzial_Kamienica;"
	exec_query_commit_PG(drop_quer)
	create_query = """create table pow_sum_dzial_Kamienica (
	 NR_KW varchar,  
	...
	drop_quer = "drop table if exists wlasciciele_Kamienica;"
	exec_query_commit_PG(drop_quer)
	create_query = """create table wlasciciele_Kamienica (
	 NR_KW varchar, 
	 NR_udzialu varchar, 	
	 Imie varchar,
	 Drugie_imie varchar,
	...
	#udzialy 
	drop_quer = "drop table if exists udzialy_Kamienica;"
	exec_query_commit_PG(drop_quer)
	create_query = """create table udzialy_Kamienica (
	 NR_KW varchar, 
	 NR_udzialu varchar, 	
	 Udzial varchar
	);"""
...
# budynki
	drop_quer = "drop table if exists budynki_Kamienica;"
	exec_query_commit_PG(drop_quer)

	create_query = """create table budynki_Kamienica (
	NR_KW varchar,
	...
	exec_query_commit_PG(create_query)
#udzialy lokalowe 
	drop_quer = "drop table if exists udz_lok_bud_Kamienica;"
	exec_query_commit_PG(drop_quer)
	create_query = """create table udz_lok_bud_Kamienica(
	NR_KW varchar,
	nr_udzialu varchar,
...
#main_dir_list = ['E:\\KW_Kamienica\\1\\']
main_dir_list = ['C:\\PB\\M_Bielsko\\KW\\dzialki\\--09_Kamienica--\\kw\\']

#polacznie z baza POSTGRES 
teraz = time.asctime( time.localtime(time.time()))
naz_log.write('START -' +teraz + '\n')
try:
	conn_PG = psycopg2.connect("dbname='--KW_09_KAMIENICA_20190916--' user='postgres' host='_host_name_' password='_passwd_'")
